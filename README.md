...
1. **Calculated Function Output**
    The backend includes a function that accepts multiple input parameters - such as recipe title, ingredients, and instructions - and returns nurtitional data by analyzing the inputs retrieved from the Spoonacular API.
2. **Persistent Internal API**
    Recipes are stored in a SQLite3 database through an internal API. The data is accessible throughout the app throught RESTful endpoints, which allows users to view, edit and delete recipes.
3. **Node.js Web Server with Express.js**
    The project uses Express.js to create a mocular and scalable we server. Routes are organized for CRUD operations and external API integration, which ensures clean separation of concerns.
4. **SQLite3 Database Interaction**
    All recipe data is stored and retrieved using SQLite3. The backend handles table creation, insertion, querying and deletion, which provides persistent storage for user-generated content.
